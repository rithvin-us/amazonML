"""Model variant from a run's saved features: retrain -> rescore saved test feats -> ce-apply (reusing a CE model).

  python variant.py <name> <src_train_run> <test_feats_run> <ce_dir> [extra pipeline args (model/blocking flags)]
...
RUNS = ROOT / "runs"
NAME, SRC, FEATS, CE, EXTRA = sys.argv[1], sys.argv[2], sys.argv[3], sys.argv[4], sys.argv[5:]
...
def step(tag: str, args: list[str]) -> str:
...
tr = step("retrain", ["retrain", "--run", SRC, "--name", NAME, *EXTRA])
